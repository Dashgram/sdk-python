...
def object_to_dict(obj, handler_type: typing.Optional[HandlerType] = None) -> dict:
    ...
    return {"update_id": update_id, handler_name: obj.json}
...
def bind(sdk, bot: typing.Union[TeleBot, AsyncTeleBot]):
    if not telebot:
        raise ImportError('pyTelegramBotAPI is not installed')

    if isinstance(bot, TeleBot):
        bot.setup_middleware(TrackMiddleware(sdk))
    elif isinstance(bot, AsyncTeleBot):
